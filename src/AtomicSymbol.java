...
public class AtomicSymbol {
  ...
  private static String[] atomicSymbols;
  ...
  static int elementToAtomicNumber(String paramString) {
    int i = 0;
    if (atomicSymbolMap.containsKey(paramString))
      i = ((Integer)atomicSymbolMap.get(paramString)).intValue(); 
    return i;
  }
  
  static String atomicNumberToSymbol(int paramInt) {
    String str = null;
    if (paramInt >= 0 && paramInt < atomicSymbols.length) {
      str = atomicSymbols[paramInt];
    } else {
      str = atomicSymbols[0];
    } 
    return str;
