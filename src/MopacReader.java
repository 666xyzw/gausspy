...
class MopacReader implements MoleculeReader {
  BufferedReader input;
  
  public MopacReader(Reader paramReader) { this.input = new BufferedReader(paramReader); }
  ...
  public void read(Molecule paramMolecule) throws Exception {
    while (this.input.ready()) {
      ...
      if (str.indexOf("ORIENTATION OF MOLECULE IN FORCE CALCULATION") >= 0) {
        for (byte b = 0; b < 3; b++)
          str = this.input.readLine(); 
        readCoordinates(paramMolecule);
        continue;
      } 
      if (str.indexOf("NORMAL COORDINATE ANALYSIS") >= 0) {
        for (byte b = 0; b < 4; b++)
          str = this.input.readLine(); 
  ...
  void readFrequencies(Molecule paramMolecule) throws Exception {
    ...
    while (str.indexOf("ROOT NO") >= 0) {
      ...
      for (b1 = 0; b1 < 15; b1++) {
        str = readLine();
        if (str.trim().length() > 0 || str.indexOf("ROOT NO") >= 0)
          break; 
      } 
    } 
    for (byte b = 0; b < 6; b++)
