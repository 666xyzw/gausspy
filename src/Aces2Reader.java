...
class Aces2Reader implements MoleculeReader {
  ...
  void readCoordinates(Molecule paramMolecule) throws Exception {
    ...
    while (this.input.ready()) {
      ...
      if (streamTokenizer.nextToken() == -2) {
        ...
        throw new Exception("Error reading coordinates at atomic number");
      } 
      atom.position = new Point3d();
      if (streamTokenizer.nextToken() == -2) {
        atom.position.x = streamTokenizer.nval * 0.529177249D;
      } else {
        throw new Exception("Error reading coordinates at x coordinate");
      } 
      if (streamTokenizer.nextToken() == -2) {
        atom.position.y = streamTokenizer.nval * 0.529177249D;
      } else {
        throw new Exception("Error reading coordinates at y coordinate");
      } 
      if (streamTokenizer.nextToken() == -2) {
        atom.position.z = streamTokenizer.nval * 0.529177249D;
      } else {
        throw new Exception("Error reading coordinates at z coordinate");
      } 
  ...
  void readFrequencies(Molecule paramMolecule) throws Exception {
    ...
    while (this.input.ready()) {
      ...
      str = this.input.readLine();
      if (str.indexOf("Gradient vector") >= 0)
        break; 
      str = this.input.readLine();
      if (str.indexOf("Normal modes") >= 0)
        break; 
    } 
  }
}
