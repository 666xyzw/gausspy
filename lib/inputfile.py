...
def inputfiles():
	...
	hva = input("Enter the heavy atom found in the coordinate system (optional): ")
	...
	for file in os.listdir(direct): #listing the files in the directory
		try:
			...
			if "xyz" in fe:
				...
				xyz_input(file, functional, fbasis, sbasis, hva, route, chrg_spin, ext)
			
			elif 'spinput' in fe:
				
				spinput_gen(file, functional, fbasis, sbasis, hva, route, chrg_spin, ext)
			
			elif "gjf" in fe:
				
				gjf_input(file, functional, fbasis, sbasis, hva, route, chrg_spin, ext)
